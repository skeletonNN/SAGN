...
import shutil
import os
os.environ["CUDA_VISIBLE_DEVICES"] = '1' # 访问第2张卡
import os.path as osp
import csv
import numpy as np

np.random.seed(1337) # 设置随机数的聚宝盆
...
def main():
    ...
    print('The number of parameters: ', total) # 计算参数量
    ...
    criterion = LabelSmoothingLoss(args.num_classes, smoothing=0.1).cuda() # 这个损失函数能够有效的提高准确率
    optimizer = optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay) # 权重衰减

    if args.monitor == 'val_acc': # 检测模型的准确率
        mode = 'max'
        monitor_op = np.greater # 检测x1是否大于x2
        best = -np.Inf # 越大越好
        str_op = 'improve' # 希望提高准确率
    elif args.monitor == 'val_loss': # 检测模型的loss
        mode = 'min'
        monitor_op = np.less # 检测x1是否小于x2
        best = np.Inf
        str_op = 'reduce'

    scheduler = MultiStepLR(optimizer, milestones=[60, 90, 110], gamma=0.1) # 学习率衰减策略
    # 数据加载
    ntu_loaders = NTUDataLoaders(args.dataset, args.case, seg=args.seg)
    train_loader = ntu_loaders.get_train_loader(args.batch_size, args.workers) # 训练数据集
    val_loader = ntu_loaders.get_val_loader(args.batch_size, args.workers)
    train_size = ntu_loaders.get_train_size() # 获取训练集的大小
    val_size = ntu_loaders.get_val_size() # 获取验证集的大小


    test_loader = ntu_loaders.get_test_loader(32, args.workers) # 加载测试机数据
...
def test(test_loader, model, checkpoint, lable_path, pred_path):
    ...
    model.load_state_dict(torch.load(checkpoint)['state_dict'],False)
    model.eval()
