...
def get_indices(performer, camera, evaluation='CS'): # 按照CS和CV的方式对数据集进行划分
...
if __name__ == '__main__':
    camera = np.loadtxt(camera_file, dtype=np.int)  # camera id: 1, 2, 3 # 表明是哪一个相机来的
    performer = np.loadtxt(performer_file, dtype=np.int)  # subject id: 1~40 # 表明是哪一个物体来的
    label = np.loadtxt(label_file, dtype=np.int) - 1  # action label: 0~59 # 表明对应的标签 设置为 0-59

    frames_cnt = np.loadtxt(frames_file, dtype=np.int)  # frames_cnt # 代表每个文件的帧有多少个 游小镇
    skes_name = np.loadtxt(skes_name_file, dtype=np.string_) # 表明可以用的帧的文件的列表

    with open(raw_skes_joints_pkl, 'rb') as fr:
        skes_joints = pickle.load(fr)  # a list

    print(len(skes_joints)) # 56578
