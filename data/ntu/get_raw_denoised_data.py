...
root_path = './'
raw_data_file = osp.join(root_path, 'raw_data', 'raw_skes_data.pkl') # 原始数据
...
actors_info_dir = osp.join(save_path, 'actors_info') # 保存演员信息
...
def denoising_by_length(ske_name, bodies_data):
    ...
    过滤长度小于或等于预定义阈值的bodyID 11 长度小于11帧的值就给取消掉啦
    """
    noise_info = str()
...
def denoising_by_spread(ske_name, bodies_data):
    ...
    for (bodyID, body_data) in new_bodies_data.items():
        ...
        if ratio >= noise_spr_thres2:  # 0.69754
            ...
            body_data['motion'] = min(motion, np.sum(np.var(joints.reshape(-1, 3), axis=0))) # 求关节的方差和与动作值的最小值赋值给body_data['motion']
...
def denoising_bodies_data(bodies_data):
    ...
    bodies_data, noise_info_len = denoising_by_length(ske_name, bodies_data) # 基于帧长去噪
...
def get_one_actor_points(body_data, num_frames):
    ...
    """
    joints = np.zeros((num_frames, 75), dtype=np.float32) # num_frames*75的矩阵
    colors = np.ones((num_frames, 1, 25, 2), dtype=np.float32) * np.nan # num_frame
    start, end = body_data['interval'][0], body_data['interval'][-1]
    joints[start:end + 1] = body_data['joints'].reshape(-1, 75) # num_frames*75的矩阵
    colors[start:end + 1, 0] = body_data['colors'] # num_frames*1*25*2
...
def get_two_actors_points(bodies_data): # 这里做的数据的预处理和融合的部分（首先先对这部分的内容进行改进）
    """
    ...
    # Arguments:
        ...
        the corresponding body_data which is also a dict with 4 keys:
          ...
          - interval: a list which records the frame indices. 间隔，记录帧索引的列表
          - motion: motion amount 运动数量

    # Return:
        joints, colors.
    """
    ske_name = bodies_data['name']
    label = int(ske_name[-2:]) # 表明是哪一个标签
    num_frames = bodies_data['num_frames']
    bodies_info = get_bodies_info(bodies_data['data']) # 取body数据
...
def get_raw_denoised_data(): # 获取去噪后的数据
    """
    ...
    print('Found %d available skeleton sequences.' % num_skes) # 56578
